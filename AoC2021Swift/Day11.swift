...
class Day11
{
  ...
  func Parse(lines:[String]) {
    ...
    for l in lines {
      ...
      var x = 0
      for c in l {
        let v = UInt8(c.asciiValue! - 48)
        gridStart[GridIndex(x:x, y:y)] = v
        x += 1
      }
      assert(x == 10)
      y += 1
  ...
  func Simulate() -> Int {
    ...
    for y in 0..<10 {
      for x in 0..<10 {
        ...
        var newValue = currentState[index] + 1
        if newValue > 9 {
          flashed[index] = true
    ...
    while (flashes.count > 0) {
      ...
      for f in flashes {
        let flashY = f / 12
        let flashX = f % 12
        for y in flashY-1...flashY+1 {
          for x in flashX-1...flashX+1 {
            let index = y*12+x
            if (!flashed[index]) {
              var newValue = currentState[index] + 1
              if newValue > 9 {
                flashed[index] = true
                newFlashes.append(index)
                flashedCount += 1
                newValue = 0
              }
              currentState[index] = newValue
  ...
  func Part1(steps:Int) -> Int {
    ...
    for y in 0..<10 {
      for x in 0..<10 {
        let index = GridIndex(x: x, y: y)
        currentState[index] = gridStart[index]
      }
    }

    var flashedCount = 0
    for _ in 0..<steps {
      flashedCount += Simulate()
    }
    return flashedCount
  }
